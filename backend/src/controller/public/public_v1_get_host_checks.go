...
func (c *Controller) reqV1GetHostChecks(w http.ResponseWriter, r *http.Request) gousuchi.IResponse {
	...
	respData, err := c.mapPgCheckV1ToAPIGetHostChecksV1ResponseBody(pgChecks, reCheckStatuses)
	if err != nil {
		return gousuchi.InternalServerError(r, "Error mapping response: %s", err)
	}

	return gousuchi.JSON(r, respData).
		WithDetailedMessage("Loaded %d checks for host %s", len(pgChecks), hostUID)
}
