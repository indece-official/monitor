...
func (c *Controller) reqV1GetUser(w http.ResponseWriter, r *http.Request) gousuchi.IResponse {
	...
	respData, err := c.mapPgUserV1ToAPIGetUserV1ResponseBody(pgUser)
	if err != nil {
		return gousuchi.InternalServerError(r, "Error mapping response: %s", err)
	}

	return gousuchi.JSON(r, respData).
		WithDetailedMessage("Loaded user %s", pgUser.UID)
}
