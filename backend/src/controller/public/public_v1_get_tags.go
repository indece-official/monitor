...
func (c *Controller) reqV1GetTags(w http.ResponseWriter, r *http.Request) gousuchi.IResponse {
	...
	respData, err := c.mapPgTagV1ToAPIGetTagsV1ResponseBody(pgTags)
	if err != nil {
		return gousuchi.InternalServerError(r, "Error mapping response: %s", err)
	}

	return gousuchi.JSON(r, respData).
		WithDetailedMessage("Loaded %d tags", len(pgTags))
}
