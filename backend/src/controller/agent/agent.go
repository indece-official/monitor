...
var (
	serverHost = flag.String("server_agent_host", "0.0.0.0", "")
	serverPort = flag.Int("server_agent_port", 9440, "")
)
...
func (c *Controller) startServer() error {
	...
	if pgConfigProperties[model.PgConfigPropertyV1KeyTLSCaCrt] == nil ||
		...
		c.log.Infof("Not starting agent server: no tls certs")
	...
	apiagent.RegisterAgentServer(c.server, c)
